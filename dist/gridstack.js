...
var GridStack = /** @class */ (function () {
    ...
    function GridStack(el, opts) {
        ...
        /** @internal */
        this.setGridWidth = obsolete(this, GridStack.prototype.column, 'setGridWidth', 'column', 'v0.5.3');
        /** @internal */
        this.setColumn = obsolete(this, GridStack.prototype.column, 'setColumn', 'column', 'v0.6.4');
        /** @internal */
        ...
        var rowAttr = Utils.toNumber(el.getAttribute('data-gs-row'));
        ...
        this._updateStyles();
        this.engine = new GridStackEngine(this.opts.column, function (cbNodes, removeDOM) {
            if (removeDOM === void 0) { removeDOM = true; }
            var maxHeight = 0;
            _this.engine.nodes.forEach(function (n) { maxHeight = Math.max(maxHeight, n.y + n.height); });
            cbNodes.forEach(function (n) {
                ...
                if (removeDOM && n._id === null) {
                    ...
                    _this._writeAttrs(el, n.x, n.y, n.width, n.height);
                }
            });
            _this._updateStyles(false, maxHeight); // false = don't recreate, just append if need be
        ...
        if (this.opts.auto) {
            ...
            elements_1.sort(function (e) { return e.i; }).forEach(function (item) { _this._prepareElement(item.el); });
        }
    ...
    GridStack.prototype.addWidget = function (els, options) {
        ...
        if (arguments.length > 2) {
            ...
            var a = arguments, i = 1, opt = { x: a[i++], y: a[i++], width: a[i++], height: a[i++], autoPosition: a[i++],
        ...
        if (options) {
            options = __assign({}, options); // make a copy before we modify in case caller re-uses it
    ...
    GridStack.prototype.save = function (saveContent) {
    ...
    GridStack.prototype.load = function (layout, addAndRemove) {
        ...
        var items = GridStack.Utils.sort(layout);
        var removed = [];
        this.batchUpdate();
        // see if any items are missing from new layout and need to be removed first
        if (addAndRemove) {
            var copyNodes = this.engine.nodes.slice(); // don't loop through array you modify
        ...
        items.forEach(function (w) {
            var item = _this.engine.nodes.find(function (n) { return n.id === w.id; });
            if (item) {
                _this.update(item.el, w.x, w.y, w.width, w.height); // TODO: full update
            }
    ...
    GridStack.prototype.getCellHeight = function (forcePixel) {
        ...
        var height = Utils.toNumber(el.getAttribute('data-gs-height'));
    ...
    GridStack.prototype.cellHeight = function (val, update) {
        ...
        var data = Utils.parseHeight(val);
    ...
    GridStack.prototype.column = function (column, doNotPropagate) {
        ...
        var domNodes = undefined; // explicitly leave not defined
    ...
    GridStack.prototype.enableMove = function (doEnable, includeNewWidgets) {
        var _this = this;
        if (includeNewWidgets === void 0) { includeNewWidgets = true; }
        if (doEnable && this.opts.staticGrid) {
            return this;
        } // can't move a static grid!
        this.getGridItems().forEach(function (el) { return _this.movable(el, doEnable); });
    ...
    GridStack.prototype.enableResize = function (doEnable, includeNewWidgets) {
        ...
        this.getGridItems().forEach(function (el) { return _this.resizable(el, doEnable); });
        if (includeNewWidgets) {
    ...
    GridStack.prototype.getCellFromPixel = function (position, useDocRelative) {
        ...
        var rowHeight = (box.height / parseInt(this.el.getAttribute('data-gs-current-row')));
     ...
     */
    GridStack.prototype.locked = function (els, val) {
     ...
     */
    GridStack.prototype.maxWidth = function (els, val) {
     ...
     * @param val A numeric value of the number of columns
     */
    GridStack.prototype.minWidth = function (els, val) {
     ...
     */
    GridStack.prototype.maxHeight = function (els, val) {
     ...
     * @param val A numeric value of the number of rows
     */
    GridStack.prototype.minHeight = function (els, val) {
    ...
    GridStack.prototype.movable = function (els, val) {
        ...
        GridStack.getElements(els).forEach(function (el) {
            ...
            if (node.noMove) {
                _this.dd.draggable(el, 'disable');
                el.classList.remove('ui-draggable-handle');
            }
            else {
                _this._prepareDragDropByNode(node); // init DD if need be
                _this.dd.draggable(el, 'enable');
                el.classList.remove('ui-draggable-handle');
            }
        });
        return this;
     ...
     */
    GridStack.prototype.move = function (els, x, y) {
        var _this = this;
        this._updateElement(els, function (el, node) {
            x = (x !== undefined) ? x : node.x;
            y = (y !== undefined) ? y : node.y;
            _this.engine.moveNode(node, x, y, node.width, node.height);
        });
        return this;
    };
    /**
     ...
     */
    GridStack.prototype.resize = function (els, width, height) {
        var _this = this;
        this._updateElement(els, function (el, node) {
            width = (width || node.width);
            height = (height || node.height);
            _this.engine.moveNode(node, node.x, node.y, width, height);
        });
        return this;
    };
    /**
    ...
    GridStack.prototype.resizable = function (els, val) {
        ...
        GridStack.getElements(els).forEach(function (el) {
            ...
            if (node.noResize) {
                _this.dd.resizable(el, 'disable');
            }
            else {
                _this._prepareDragDropByNode(node); // init DD if need be
                _this.dd.resizable(el, 'enable');
            }
        });
        return this;
    };
     ...
     */
    GridStack.prototype.setStatic = function (val) {
        var _this = this;
        ...
        if (val) {
            this.getGridItems().forEach(function (el) { return _this.dd.remove(el); });
        }
        else {
            this.engine.nodes.forEach(function (n) { return _this._prepareDragDropByNode(n); });
        }
        this._setStaticClass();
        return this;
    };
     ...
     * @param height  new dimensions height. If value is null or undefined it will be ignored.
     */
    GridStack.prototype.update = function (els, x, y, width, height) {
        var _this = this;
        this._updateElement(els, function (el, node) {
            x = (x !== undefined) ? x : node.x;
            y = (y !== undefined) ? y : node.y;
            width = (width || node.width);
            height = (height || node.height);
            _this.engine.moveNode(node, x, y, width, height);
        });
        return this;
    };
    /**
    ...
    GridStack.prototype.margin = function (value) {
        var data = Utils.parseHeight(value);
    ...
    GridStack.prototype.willItFit = function (x, y, width, height, autoPosition) {
        return this.engine.canBePlacedWithRespectToHeight({ x: x, y: y, width: width, height: height, autoPosition: autoPosition });
    };
    /** @internal */
    GridStack.prototype._triggerChangeEvent = function (skipLayoutChange) {
    ...
    GridStack.prototype._updateStyles = function (forceUpdate, maxHeight) {
        ...
        if (!this._styles) {
            ...
            var placeholder = "." + this.opts._class + " > .grid-stack-placeholder > .placeholder-content";
            Utils.addCSSRule(this._styles, content, "top: " + top_1 + "; right: " + right + "; bottom: " + bottom + "; left: " + left + ";");
            Utils.addCSSRule(this._styles, placeholder, "top: " + top_1 + "; right: " + right + "; bottom: " + bottom + "; left: " + left + ";");
        ...
        if (maxHeight > this._styles._max) {
            ...
            for (var i = this._styles._max + 1; i <= maxHeight; i++) { // start at 1
    ...
    GridStack.prototype._updateContainerHeight = function () {
        ...
        var row = this.getRow(); // checks for minRow already
    ...
    GridStack.prototype._prepareDragDropByNode = function (node) {
        ...
        var onStartMoving = function (event, ui) {
            ...
            _this.engine.beginUpdate(node);
            cellWidth = _this.cellWidth();
            cellHeight = _this.getCellHeight(true); // force pixels for calculations
            _this.placeholder.setAttribute('data-gs-x', target.getAttribute('data-gs-x'));
            _this.placeholder.setAttribute('data-gs-y', target.getAttribute('data-gs-y'));
            _this.placeholder.setAttribute('data-gs-width', target.getAttribute('data-gs-width'));
            _this.placeholder.setAttribute('data-gs-height', target.getAttribute('data-gs-height'));
            _this.el.append(_this.placeholder);
            node.el = _this.placeholder;
            node._beforeDragX = node.x;
            node._beforeDragY = node.y;
            node._prevYPix = ui.position.top;
            _this.dd.resizable(el, 'option', 'minWidth', cellWidth * (node.minWidth || 1));
            _this.dd.resizable(el, 'option', 'minHeight', cellHeight * (node.minHeight || 1));
        };
        ...
        var dragOrResize = function (event, ui) {
            ...
            if (event.type === 'drag') {
                ...
                if (el.dataset.inTrashZone || x < 0 || x >= _this.engine.column || y < 0 || (!_this.engine.float && y > _this.engine.getRow()) || node._added) {
                    if (node._temporaryRemoved) {
                        return;
                    }
                    if (_this.opts.removable === true) {
                        _this._setupRemovingTimeout(el);
                    }
                    x = node._beforeDragX;
                    y = node._beforeDragY;
                    if (_this.placeholder.parentNode === _this.el) {
                    ...
                    _this._clearRemovingTimeout(el);
                    if (node._temporaryRemoved) {
                        _this.engine.addNode(node);
                        _this._writeAttrs(_this.placeholder, x, y, width, height);
                        _this.el.appendChild(_this.placeholder);
                        node.el = _this.placeholder;
                ...
                height = Math.round((ui.size.height + _this.getMargin()) / cellHeight);
            }
            // width and height are undefined if not resizing
            var _lastTriedWidth = (width || node._lastTriedWidth);
            var _lastTriedHeight = (height || node._lastTriedHeight);
            if (!_this.engine.canMoveNode(node, x, y, width, height) ||
        ...
        var onEndMoving = function (event) {
            ...
            if (node._isAboutToRemove) {
                ...
                if (!node._temporaryRemoved) {
                    ...
                    _this._writeAttrs(target, node.x, node.y, node.width, node.height);
                }
                else {
                    Utils.removePositioningStyles(target);
                    _this._writeAttrs(target, node._beforeDragX, node._beforeDragY, node.width, node.height);
            ...
            if (event.type === 'resizestop') {
                _this._resizeNestedGrids(target);
            }
        };
        this.dd
            .draggable(el, {
    ...
    GridStack.prototype._resizeNestedGrids = function (target) {
        target.querySelectorAll('.grid-stack').forEach(function (el) {
            if (el.gridstack) {
    ...
    GridStack.prototype._prepareElement = function (el, triggerAddEvent) {
        ...
        var node = this._readAttr(el, { el: el, grid: this });
    ...
    GridStack.prototype._writeAttrs = function (el, x, y, width, height) {
    ...
    /** @internal */
    GridStack.prototype._updateElement = function (els, callback) {
        var el = GridStack.getElement(els);
        if (!el) {
            return this;
        }
    ...
    GridStack.prototype.onParentResize = function () {
        ...
        if (this._isAutoCellHeight) {
            Utils.throttle(function () {
                var marginDiff = -_this.opts.marginRight - _this.opts.marginLeft
                    + _this.opts.marginTop + _this.opts.marginBottom;
                _this.cellHeight(_this.cellWidth() + marginDiff);
            }, 100);
        }
    ...
    GridStack.prototype._updateWindowResizeEvent = function (forceRemove) {
        ...
        var workTodo = (this._isAutoCellHeight || !this.opts.disableOneColumnMode);
    ...
    GridStack.prototype._setupDragIn = function () {
    ...
    GridStack.prototype._setupRemoveDrop = function () {
        ...
        if (!this.opts.staticGrid && typeof this.opts.removable === 'string') {
            var trashZone = document.querySelector(this.opts.removable);
            if (!trashZone)
                return this;
            ...
            this.dd
                .on(trashZone, 'dropover', function (event, el) {
                var node = el.gridstackNode;
                if (!node || node.grid !== _this)
                    return;
                el.dataset.inTrashZone = 'true';
                _this._setupRemovingTimeout(el);
            })
                .on(trashZone, 'dropout', function (event, el) {
                var node = el.gridstackNode;
                if (!node || node.grid !== _this)
                    return;
    ...
    GridStack.prototype._setupAcceptWidget = function () {
        ...
        if (this.opts.staticGrid || !this.opts.acceptWidgets)
            return this;
        var onDrag = function (event, el) {
            var node = el.gridstackNode;
            var pos = _this.getCellFromPixel({ left: event.pageX, top: event.pageY }, true);
            ...
            if (!node._added) {
                ...
                _this.engine.beginUpdate(node);
                _this.engine.addNode(node);
                _this._writeAttrs(_this.placeholder, node.x, node.y, node.width, node.height);
                _this.el.appendChild(_this.placeholder);
                node.el = _this.placeholder; // dom we update while dragging...
                node._beforeDragX = node.x;
                node._beforeDragY = node.y;
                _this._updateContainerHeight();
            }
            else if ((x !== node.x || y !== node.y) && _this.engine.canMoveNode(node, x, y)) {
                _this.engine.moveNode(node, x, y);
                _this._updateContainerHeight();
            }
        };
        this.dd
            ...
            var width = node.width || Math.round(el.offsetWidth / _this.cellWidth()) || 1;
            var height = node.height || Math.round(el.offsetHeight / _this.getCellHeight(true)) || 1;
            // copy the node original values (min/max/id/etc...) but override width/height/other flags which are this grid specific
            var newNode = _this.engine.prepareNode(__assign({}, node, { width: width, height: height, _added: false, _temporary: true }));
            newNode._isOutOfGrid = true;
            el.gridstackNode = newNode;
            el._gridstackNodeOrig = node;
            _this.dd.on(el, 'drag', onDrag);
            return false; // prevent parent from receiving msg (which may be grid as well)
        })
            .on(this.el, 'dropout', function (event, el) {
            ...
            if (origNode && origNode.grid && origNode.grid !== _this) {
            ...
            var node = el.gridstackNode; // use existing placeholder node as it's already in our list with drop location
            var _id = node._id;
            _this.engine.cleanupNode(node); // removes all internal _xyz values (including the _id so add that back)
            node._id = _id;
            node.grid = _this;
            _this.dd.off(el, 'drag');
            ...
            if (_this._gsEventHandler['dropped']) {
                _this._gsEventHandler['dropped']({ type: 'dropped' }, origNode && origNode.grid ? origNode : undefined, node);
            }
    ...
    GridStack.prototype.initMargin = function () {
        var data = Utils.parseHeight(this.opts.margin);
        this.opts.marginUnit = data.unit;
        var margin = this.opts.margin = data.height;
    ...
    GridStack.prototype._updateAttr = function (els, val, attr, field) {
        GridStack.getElements(els).forEach(function (el) {
            if (val) {
